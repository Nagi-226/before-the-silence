...
| 武器 | 类型 | 弹匣（基础/一级/二级） | 单发伤害（基础/一级/二级） | 获取方式 |
| --- | --- | --- | --- | --- |
| 手枪 | 半自动 | 20 / 25 / 30 发 | 32 / 40 / 50 | 初始持有 |
| 冲锋枪 | 全自动 | 30 / 40 / 50 发 | 32 / 40 / 50 | 场景拾取（金色光晕标识） |

弹匣与伤害的一二级数值由**通用武器升级组件**提供（每张地图一级、二级各 1 个，需按顺序获取：二级组件要求先完成一级改装）。
...
| W | 冲锋枪 | 获得冲锋枪 |
| u | 一级武器升级组件 | 全武器弹匣扩容 + 伤害提升（青色光晕标识） |
| U | 二级武器升级组件 | 全武器再强化（橙色光晕标识，需先完成一级改装） |
...
全武器基础单发伤害 32，经升级组件可提升至 40 / 50。Godot 版每张地图生成 136 个敌人、385+ 个拾取物。
