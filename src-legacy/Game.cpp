...
Game::Game(SDL_Window* window, SDL_Renderer* renderer, int windowWidth, int windowHeight) :
    ...
    if (window != nullptr && renderer != nullptr) {
        ...
        //Initialize SDL_ttf for true UTF-8 text rendering (Chinese supported).
        if (TTF_WasInit() == 0) {
            if (TTF_Init() != 0) {
        ...
        //Try common Chinese-capable fonts on Windows first, then local project fallback.
        ...
        for (const auto& fontPath : fontCandidates) {
            fontUi = TTF_OpenFont(fontPath.c_str(), 12);
            if (fontUi != nullptr)
                break;
        }

        if (fontUi == nullptr) {
            std::cout << "Warning: Couldn't load a Chinese-capable font. Text rendering may fail = "
...
void Game::drawOverlayVictory(SDL_Renderer* renderer) {
    ...
    //Draw the text.
    drawText(renderer, 53, 53, 3, "胜利!");
    drawText(renderer, 69, 89, 1, "-按ESC退出-");
}
...
void Game::drawText(SDL_Renderer* renderer, int offsetX, int offsetY, int size, std::string textToDraw) {
    if (renderer == nullptr || fontUi == nullptr || textToDraw.empty())
        return;

    //Render UTF-8 text (supports Chinese and ASCII mixed strings).
    SDL_Color colorText{ 255, 255, 255, 255 };
    SDL_Surface* surfaceText = TTF_RenderUTF8_Blended(fontUi, textToDraw.c_str(), colorText);
    ...
    SDL_Rect rDst{ offsetX, offsetY, surfaceText->w * size, surfaceText->h * size };
    SDL_RenderCopy(renderer, textureText, nullptr, &rDst);
