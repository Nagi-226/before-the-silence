...
func _test_campaign_flow() -> void:
	...
	# 墙格从配置推导(双线合并后统一口径): 围界环 + 建筑周界 - 门洞。
	# 存量断言升级为配置驱动(非松绑)——map2 街区布局再演进无需改断言。
	var m2cfg := {}
	...
	# 双线汇合点: 第二关的 v 由 B批3 走 level_ext.maps[map=2].pickups 配置在东北
	# 土丘顶, 标记通路来自 A批8 的 MiniMap.target_marks(MARK_COLORS 含 v=白菱形)。
	# (weapons.json 的 upgradeComponents 标 map=1 属第三关——campaign.sequence=
	# [0,2,1], map1 地下隐匿设施是终点关, 索引语义正确, 非缺陷。)
	var mk2 := {}
	var mk2_floor := {}
	if mm2 != null:
		for m2 in mm2.target_marks():
			...
			mk2_floor[str(m2d["key"])] = int(m2d["floor"])
	var v_cfg := {}
	for pc in m2cfg.get("pickups", []):
		var pcd: Dictionary = pc
		if str(pcd.get("symbol", "")) == "v":
			v_cfg = pcd
	_report(not v_cfg.is_empty(), "map2 配置含三阶组件 v (B批3 丘顶奖励级补给)")
	_report(mk2.has("v") and mk2["v"] == Vector2i(int(v_cfg.get("x", -1)),
			int(v_cfg.get("y", -1))) and int(mk2_floor.get("v", -1)) == 1,
		"map2 三阶组件 v 标记落位: %s (期望配置 (%s,%s) 丘顶), 楼层=%s (期望 1, 室外单层图)"
...
func _finish() -> void:
	while not _noop_test_done or not _cycle_test_done or not _components_test_done \
			...
			or not _layer_test_done or not _minimap_floor_test_done \
			or not _gate_static_test_done:
		await get_tree().physics_frame
